...
export default class FsProcessCreditScreen extends NavigationMixin(LightningElement) {
    ...
    // @wire(getCapabilitySummary, { applicationId: this.applicationId })
    // wiredCapabilityRecords(value) {
    //     const { data, error } = value;
    //     this._wiredCapability = value;
    //     if (data) {
    ...
    rowselectionevent(event) {
        ...
        if (detail === 'Generate_CAM_Report') {
            ...
            this[NavigationMixin.GenerateUrl]({
                ...
                attributes: {
                    url: '/apex/CAMReportVf?id=' + this.applicationId
                }
            }).then(vfURL => {
                window.open(vfURL);
            });
                ...
                if (result && result.length) {
                    ...
                    result.forEach(element => {
                        this.propertySummaryObj.propertyList.push({
                            ...
                            Building_Area_Sq_Ft__c: (element.Building_Area_Sq_Ft__c != undefined) ? element.Building_Area_Sq_Ft__c : 0,
                            Building_Value_per_Sq_ft__c: (element.Building_Value_per_Sq_ft__c != undefined) ? element.Building_Value_per_Sq_ft__c : 0,
