...
import searchLookupRecords from '@salesforce/apex/LWCLookupCtrl.searchLookupRecords'; // Apex method 
import getRecentlyCreatedRecord from '@salesforce/apex/LWCLookupCtrl.getRecentlyCreatedRecord'; // Apex method 
import getSelectedRecord from '@salesforce/apex/LWCLookupCtrl.getSelectedRecord'; // Apex method 
...
export default class LWCLookup extends LightningElement {
    ...
    @api iconName = 'standard:account'; // Icon Name (i.g: standard:account) 
    ...
    */
    ...
    connectedCallback() {
        if (this.seletedRecordId) {
            this.getPreSelectedRecord(this.seletedRecordId);
        }
    }

    get showLabel(){
        return (this.variant == 'label-hidden' ? false : true);
    }

    @api
    getPreSelectedRecord(recordId) {
        getSelectedRecord({
            ...
            console.log('result in getSelectedRecord= ', result);
            if (result) {
                this.value = [result];
            }
        }).catch((err) => {
            console.log('Error in getSelectedRecord= ', result);
        });
    }
